...
describe("atc-rag Vercel deployment root", () => {
  it("uses the Vercel project's configured apps/rag root exactly once", () => {
    ...
    expect(betaJob).toContain("vercel deploy \\");
    expect(betaJob).toContain('--token="$VERCEL_TOKEN"');
    expect(betaJob).toContain("--yes");
    ...
    expect(productionStep).toContain("vercel deploy \\");
    expect(productionStep).toContain("--prod");
    expect(productionStep).toContain("--yes");
    expect(productionStep).toContain('--token="$VERCEL_TOKEN"');
