# AGENTS.md — AI Travel Concierge

Working instructions for Codex sessions in this repo. Read this first, every session, before doing anything else.
...
Use `/memory-entry` — it owns the entry format and the prepend mechanics. A PreToolUse hook (`.Codex/hooks/block-spec-memory-edits.mjs`) rejects any MEMORY.md change that isn't a pure prepend, and **fails closed** — if an edit gets blocked, read the skill rather than fighting the hook. Do not reword or replace a prior entry; if you truly must, ask the user for explicit permission first.
...
Only the orchestrator writes `MEMORY.md`, `MEMORY-INDEX.md`, `SESSION.md`, `AGENTS.md`, or `docs/runbooks/**`. Subagents run in worktrees and in parallel: two of them independently computing "highest D-number + 1" collide (#1661), and instruction changes scattered across feature PRs never get reviewed as a set.
...
### Model-tier compatibility mapping

The GitHub labels `haiku`, `sonnet`, `opus`, and `fable` are durable workload-tier
names inherited from the existing issue taxonomy. Preserve those exact names when
...
**Model selection for audit runs (D-317):** default to the `sonnet` tier; risk triggers and large diffs escalate to the `opus` tier — full criteria in `pr-workflow.md` and Codex profile mapping above.
...
- Prune your own agent worktrees under `.Codex/worktrees/` (leave any holding unpushed work — that's the user's call)
