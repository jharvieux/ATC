...
### §14.11 — 1099-NEC reporting (MISSING entirely)
- **Spec:** Annual 1099-NEC issuance for sub-host subcontractors paid > $600/year via the platform. IRS compliance requirement for any tenant of the platform paying U.S. subcontractors.
- **Current state:** **Zero code matches `1099`, `1099-NEC`, `nec_reporting`**. No table, no cron, no export.
- **Risk:** IRS penalty for non-filing once a sub-host hits the threshold. Up to $310/form, capped per filer.
- **Action:** Build before any sub-host tenant crosses $600 of subcontractor payments in a calendar year. At launch, the simplest path is a January cron that exports a CSV per tenant and emails the operator for manual e-file via a 1099 service (Track1099, Tax1099). Full e-file integration can come later.
...
### §29.14 — Disaster Recovery Posture (no runbook)
- **Spec:** Documented DR procedures: RPO/RTO targets, backup verification cadence, recovery rehearsal log.
- **Current state:** No `docs/runbooks/disaster-recovery.md` or similar. We have `rollback-database.md` + `rollback-application.md` (partial coverage), but no full-loss DR runbook with Supabase point-in-time recovery + Vercel restore procedures.
- **Action:** Write `docs/runbooks/disaster-recovery.md`. Required before SOC 2 audit (referenced in §26.12).

### §30.7 — Load testing (k6 scripts missing)
- **Spec:** k6 load tests for the 6 §30.7 scenarios (homepage, chat SSE, statement upload, etc.), run out-of-band per release.
- **Current state:** **Zero `.k6.js` or `k6.config.*` files in the repo.** MEMORY D-064 mentioned k6 scripts were "shipped"; they're absent on dev today.
- **Action:** Either restore the k6 scripts (check git history for them under `tests/load/`) or rewrite. Without these the perf-budget claims in §30.7 aren't verifiable.
