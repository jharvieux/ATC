...
skip_table: pricing_seat_ladder         -- REASON: EPIC #1336 Phase 1 — global agency seat ladder, no tenant_id (PLATFORM_READABLE). RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads (lib/pricing/pricing-table.ts) + writes (Phase 3 editor). No authenticated/SSR read path; USING(true) barred by the lint gate.
skip_table: stripe_price_map            -- REASON: EPIC #1336 Phase 2 — Stripe Price-ID map, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads (priceIdFor/loadPriceMap) + writes (backfill script + Phase 3 editor). No authenticated/SSR read path; USING(true) barred by the lint gate.
...
skip_table: apify_spend_ledger          -- REASON: #1052 / BP34 §33.9.3 — platform-scoped Apify spend ledger, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads/writes by the ApifyPricingAdapter.
skip_table: pricing_cache               -- REASON: #1052 / BP33 §33.3 — platform-scoped pricing reference, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads/writes by the pricing adapter.
skip_table: cruisemapper_url_inventory  -- REASON: #1052 / BP36 §33.5 — platform-scoped URL inventory for the DIY CruiseMapper scraper, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only writes by the Inngest scraper job.
skip_table: destination_images          -- REASON: #1052 — platform-scoped destination image catalog, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads/writes by the groups hero-image helper (lib/groups/hero-image.ts).
skip_table: destination_images_cache    -- REASON: #1052 — platform-scoped per-query image cache, no tenant_id. RLS enabled with zero policies = default-deny over the Data API. Service-role-only reads/writes/upserts by the groups hero-image helper (lib/groups/hero-image.ts).
