...
skip_table: schema_migrations           -- REASON: #1052 — db-migrate ledger (scripts/db-migrate.ts), no tenant_id. RLS enabled with zero policies = default-deny over the Data API; the migrate runner connects as the owner/superuser (bypasses RLS). No authenticated/anon access path.
