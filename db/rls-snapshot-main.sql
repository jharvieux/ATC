...
-- public.apify_spend_ledger (rls_enabled)
...
CREATE POLICY "bug_submissions_select" ON public.bug_submissions
  ...
  USING ((submitter_user_id IN ( SELECT users.id
   FROM users
  WHERE users.auth_user_id = (( SELECT auth.uid() AS uid)))) OR auth_user_in_tenant(tenant_id));
...
CREATE POLICY "canonical_match_reviews_platform_admin_read" ON public.canonical_match_reviews
  ...
  WHERE platform_admins.auth_user_id = (( SELECT auth.uid() AS uid)))));
...
CREATE POLICY "conversations_insert_policy" ON public.conversations
  ...
  WITH CHECK (auth_user_in_tenant(tenant_id) AND tenant_is_active(tenant_id) AND user_id = (( SELECT u.id
   FROM users u
  WHERE u.auth_user_id = (( SELECT auth.uid() AS uid)) AND u.tenant_id = conversations.tenant_id AND u.status = 'active'::text)));
...
-- TABLE: public.email_retry_content
