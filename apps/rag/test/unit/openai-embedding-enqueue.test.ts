...
describe("enqueueEmbedding", () => {
  it("inserts one row with a generated custom_id and returns ids", async () => {
    ...
    const out = await enqueueEmbedding({ chunk_id: "chunk-1", content: "hello world", db });
  ...
  it("trims content before insert", async () => {
    const { db, calls } = mockDb();
    await enqueueEmbedding({ chunk_id: "c", content: "  trimmed  ", db });
    const payload = calls[0]?.payload as Record<string, unknown>;
  ...
  it("rejects empty content", async () => {
    const { db } = mockDb();
    await expect(
      enqueueEmbedding({ chunk_id: "c", content: "   ", db }),
  ...
  it("rejects missing chunk_id", async () => {
    const { db } = mockDb();
    await expect(
      enqueueEmbedding({ chunk_id: "", content: "x", db }),
  ...
  it("surfaces Supabase errors as a structured throw", async () => {
    ...
    await expect(
      enqueueEmbedding({ chunk_id: "c", content: "x", db }),
  ...
  it("produces a unique custom_id across calls", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      const { db, calls } = mockDb();
      await enqueueEmbedding({ chunk_id: `c${i}`, content: "x", db });
