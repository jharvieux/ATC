...
export async function checkFeedbackRateLimit(
  ...
  try {
    ...
    // REDIS_URL not set (local dev). Fail open — better to log a warning
    // than to break the endpoint when redis isn't available.
    console.warn("[feedback-rate-limit] REDIS_URL not set — fail-open");
