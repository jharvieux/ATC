...
async function applyCompletedBatch(args: {
  ...
  // Bulk-flip statuses: one UPDATE for all successes, one per distinct failure
  // reason — instead of one round-trip per row.
  if (successes.length > 0) {
    await safeAwait(
      db
        .from("pending_embedding")
        .update({ status: "done", output_file_id: status.output_file_id, completed_at: completedAt })
        .in(
          "id",
