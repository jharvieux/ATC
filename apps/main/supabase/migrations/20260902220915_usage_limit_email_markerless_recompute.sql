-- Migration: usage_limit_email_markerless_recompute
-- Version:   20260902220915
-- Generated: 2026-09-02T22:09:15Z by scripts/new-migration.sh
-- Branch:    feature/sweep-abuse-state-2112
-- Worktree:  atc-sweep-abuse-state-2112
--
-- Recompute an authorized email downgrade even when lazy daily reset leaves
-- the locked metrics row on a prior UTC day and no current-day marker exists.
-- This evaluates the effective current-day value as zero without mutating the
-- stored day/count or creating a marker. Same-day markerless recompute still
-- uses the locked daily count; explicit-day recovery remains marker-bound.
--
-- Rollback: restore advance_tenant_usage_state from 20260902214154.
...
BEGIN
  ...
  IF p_dimension = 'email_volume' THEN
    ...
    ELSIF p_evaluation_day IS NULL
       AND p_allow_downgrade
       AND v_metrics.email_sent_day_ref < v_effective_day THEN
      v_metric := 0;
      v_current_state := v_metrics.email_volume_limit_state;
    ELSE
      RETURN;
