-- §14.9 — Thread Stripe event_id into process_transfer_reversal notes (#1227).
--
-- Adds p_stripe_event_id TEXT as a third parameter.  Both first-pass and
-- second-pass reconciliation_review_queue INSERT notes now include
-- 'stripe_event_id' so ops can join directly on stripe_webhook_events instead
-- of doing a manual transfer-ID scan when multiple partial reversals occur on
-- the same transfer_id (issue #1156 scenario).
--
-- Drop the previous 2-parameter signature: CREATE OR REPLACE with a different
-- argument list creates a new overload rather than replacing the old one, which
-- would leave the old (TEXT, BIGINT) function callable and grant-listed.

DROP FUNCTION IF EXISTS public.process_transfer_reversal(TEXT, BIGINT);
