...
export const ragExtractContent = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    if (result.status === "extracted" && result.content) {
      ...
      const capped = originalLen > MAX_EXTRACTED_CHARS;
      if (capped) {
        console.warn("[rag-extract-content] extracted content capped", {
          submission_id, original_chars: originalLen, cap: MAX_EXTRACTED_CHARS,
        });
      }
      const content = capped ? result.content.slice(0, MAX_EXTRACTED_CHARS) : result.content;
      await safeAwait(db
        .from("rag_submissions")
        .update({
