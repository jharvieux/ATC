// BP40 §40.8 — Tier gating: BYO Research blocked.

import type { SupabaseClient } from "@supabase/supabase-js";

const BLOCKED: ReadonlySet<string> = new Set(["byo_research"]);

export async function assertLineItemsAvailable(
  tenant_id: string,
  ...
  if (BLOCKED.has(code)) return { ok: false, reason: `line_items_not_available_for_tier:${code}` };
  return { ok: true };
}
