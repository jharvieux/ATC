...
// Resend Hobby tier: $1.90/1k emails = 0.19¢ each. Integer cents × 100 for
// fractional-cent precision: store as 19 = 0.19¢.
...
export async function GET(req: Request): Promise<Response> {
  ...
  void adminUserId; // verified above; audit log written by withPlatformAdminAudit
  ...
  const tenantProximity: TenantRow[] = (
    ...
    .map((m) => {
      ...
      const rawCost = parseBigIntCol(m.ai_cost_cents);
      return {
        tenant_id: m.tenant_id,
        slug: info.slug,
        display_name: info.display_name,
        ai_cost_cents: rawCost,
  ...
  const weatherMonth = (weatherHistoryResult.data ?? []).reduce(
    (s, r) => s + (r as { requests_count: number }).requests_count,
    0,
  );
