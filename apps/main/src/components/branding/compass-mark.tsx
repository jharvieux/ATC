// Shared compass-mark SVG geometry for Logo.tsx and LogoMark.tsx (#1610).
// Both components render the same icon (gradients/clipPath + light/dark
// variant groups) — Logo.tsx additionally appends a wordmark group inside
// each variant. Factored out so brand changes to the mark only need to
// happen once.

import * as React from "react";

export interface CompassMarkIds {
  seaGradId: string;
  horizonId: string;
  clipId: string;
}

export function CompassMarkDefs({ seaGradId, horizonId, clipId }: CompassMarkIds): React.ReactElement {
  return (
        <defs>
...
export function CompassMarkLight({ seaGradId, horizonId, clipId }: CompassMarkIds): React.ReactElement {
  return (
    <>
...
export function CompassMarkDark({ clipId }: Pick<CompassMarkIds, "clipId">): React.ReactElement {
  return (
    <>
