...
One line per older decision in `MEMORY.md`, newest first. These are decisions
whose one-liners no longer earn a slot in the session-start read
(`MEMORY-INDEX.md`): shipped-and-done work, superseded decisions, and
historical context. The full entries remain in `MEMORY.md` — grep there for
detail. Never load this file at session start.

Rebuild this file anytime it drifts (everything in MEMORY.md that the lean
index doesn't carry):

```bash
{ sed -n '1,/^## Entries$/p' MEMORY-INDEX-ARCHIVE.md; echo; \
  grep -E '^## D-' MEMORY.md | sed 's/^## /- /' \
  | awk 'NR==FNR { if ($0 ~ /^- D-/) k[$2]; next } !($2 in k)' MEMORY-INDEX.md -; } \
  > /tmp/atc-archive.$$ && mv /tmp/atc-archive.$$ MEMORY-INDEX-ARCHIVE.md
```
