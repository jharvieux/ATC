...
<!-- sync-token: 2 — bump in BOTH copies (this file and ~/.claude/commands/issue-sweep.md) with every shared-lesson change -->
...
- **End with the sweep's net ledger: `closed N / filed K (M of the closures stale) / reopened R → net ±X`, where the headline net is the TRACKER'S ACTUAL CHANGE: `open at end − open at start`, cross-checked as `closed − filed`.** Query both from the forge at the moment you report (`gh search issues --repo <r> "closed:YYYY-MM-DD"` and the matching `created:`), never from a hand-kept tally and never from a count you computed earlier in the sweep — issues you file during wrap-up land after that number is taken, which is exactly how it goes stale.

  **One basis on both sides — this is the rule, and it has been broken.** An issue the sweep both FILED and CLOSED must be counted in `N` and `K` together, or excluded from both. Do not count it as a closure while excluding it from the filings: that inflates the result by exactly the size of the fold-in set, always in the flattering direction, so the more a sweep folds in the better it scores for free. Both consistent treatments give the same net — if yours differ, the asymmetry is the bug. (MEASURED 2026-07-30: a sweep reported `net −8` where both consistent methods gave **+2, a tracker that grew**; the method alone accounted for 6 of the gap and stale inputs for the rest. It was defended as "defensible" when first questioned. It is not.)

  A healthy sweep is net-negative. **State the direction in words, not just a sign** — "189 open, up 2 on the day" cannot be misread the way "net −8" can. If filed ≥ closed, say so plainly and name which filed issues were audit findings or en-route noticings that passed the inline-fix criteria, and which eligible fold-in candidates you didn't work and why — each of those is a miss against this skill, not a judgment call. A tracker that grew because independent verification found real defects is a *good* outcome reported honestly; dressing it as a reduction is not.
