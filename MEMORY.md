...
**Why.**
- Separate producer and consumer jobs released the shared lock between revision establishment and acceptance, allowing another branch to reset or migrate the database first.
- Cancelling an active reset or staging holder can leave shared state partially rebuilt, so active holders are preserved even though GitHub may supersede the older pending run when a third arrives.
- A mutable shared application URL cannot honestly prove it runs the event SHA, and probing non-API paths or accepting blanket 404s produced false confidence rather than live isolation evidence.

**Rejected.**
- *Give every database job the same concurrency group.* Rejected because the lock is released between jobs, leaving the original interleaving seam.
- *Cancel the active holder for newer revisions.* Rejected because cancellation can strand partially mutated shared state; a superseded pending run must instead be rerun and its receipt fails closed.
- *Attribute the shared host to the event SHA without deployment evidence.* Rejected because only release staging verifies its existing health response against `GITHUB_SHA`; broader per-event and production/RAG hosted attestation remains tracked by #2122.
- *Treat every 2xx as a leak or every 404 as safe.* Rejected because legitimate tenant-B collections can return 2xx, while wrong paths and hosts can return blanket 404s; route-matched fixtures and response evidence distinguish these cases.

**Related artifacts.** Issue #2098, follow-up #2122, `.github/workflows/deploy.yml`, `scripts/enumerate-api-routes.ts`, `tests/security/cross-tenant-probe.test.ts`, `tests/security/fixtures/cross-tenant-setup.ts`, `tests/unit/workflows/deploy-test-db-provenance.test.ts`.
...
**Why**: a partial predicate keyed on a non-existent enum value is a silent no-op that reviewers would later mistake for intentional narrowing; and auto-deleting duplicate commissions/contacts/bookings at migration time is unsafe (money implications + ~15 FK cascades), so the CREATE UNIQUE INDEX is intended to fail-loud with 23505 on prod until the operator reconciles.

**Rejected**: keeping the `WHERE status <> 'reversed'` predicate (references a value that can't exist); auto-dedup inside the migration (money/FK-cascade risk); a single monolithic remediation issue (the three surfaces have different owners and risk profiles).

**Related artifacts**: PR #1630; migration `20260721000000_unique_constraints_double_payout.sql`; issues #1575, #1632, #1576, #1577, #1673; index-suffix convention note added to `docs/runbooks/migrations.md` (§8, go-forward `_uidx` for unique indexes; pre-existing suffix variants accepted, not retroactively renamed).

---

## D-325 — 2026-07-08 — /issue-sweep hardened after the 2026-07-07 sweep failures (ledger, gate, Sonnet supervisor)

**Decision**: both issue-sweep skills (ATC `.claude/commands/issue-sweep.md` and the user-level portable variant) rebuilt around a runtime state file — `.git/issue-sweep-ledger.json` (inside `.git/` so it is uncommittable by construction) holding phase, a durable `approval` field, and a per-batch state machine (`queued → executing → pr-open → ci-wait → audited → merged | parked`). Changes: (1) re-entrancy guard — a replayed `/issue-sweep` (remote-control reconnects / background-agent completions re-present the command) resumes from the ledger instead of re-triaging, and **never counts as plan approval**; execution may not start while `approval` is null (only a verbatim operator reply fills it — notifications, hooks, and silence never do). (2) Dispatch is a top-up loop: max 3 batches `executing`, never all batches in one message, advance the merge train ≤1 PR per wake-up. (3) Completion criterion: sweep unfinished while any batch is non-terminal; reconcile ledger vs `gh pr list --label auto-triaged --state open` before wrap-up. (4) Diff-hash clarified: update-branch merge commits don't change the PR diff, so audit markers normally stay valid — `post-audit-comment.sh --check` first, re-run agents only if stale. (5) Grouping coarsens to ≤6 batches. (6) Supervisor runs at Sonnet (self-check warns if launched as Haiku) and never hand-edits code — conflicts/CI failures/audit findings go to fix agents at `max(batch model, sonnet)`.

**Why**: the 2026-07-07 sweep (run with a Haiku supervisor) launched all 12 batches simultaneously despite the written cap, stalled at driving PRs to merge, re-ran audit agents needlessly after every update-branch, and treated harness-replayed commands and operator silence as gate approval — hours of cleanup. Root cause: prose rules with no runtime state, plus Haiku-grade judgment holding autonomous merge authority into dev.

**Rejected**: keeping a Haiku supervisor with an escalate-when-stuck rule (recognizing "stuck" is exactly the judgment Haiku lacks); a gitignored `.claude/` ledger path (needs a gitignore entry in every repo the portable variant visits — `.git/` placement needs none).

**Related artifacts**: this PR; supersedes the dispatch/finalization mechanics of D-314 (plan-gate, triage rubric, and model tiers unchanged); merge-train discipline #1671 / `docs/runbooks/pr-workflow.md`.

---

## D-324 — 2026-07-07 — Help-doc screenshots: manifest-driven Playwright pipeline, captured against a dedicated beta demo tenant

**Decision**: the 15 customer-facing help docs (`apps/main/content/help/`) get their 30 never-filled `[Screenshot: ...]` placeholders replaced with **generated, annotated screenshots**: a manifest (`scripts/help-screenshots/manifest.ts`) declares each shot (page, state-prep, indigo box/callout annotations as CSS selectors), `pnpm help:screenshots` regenerates all PNGs into `apps/main/public/help/<doc>/<id>.png` via Playwright + DOM-injected overlays, and a `check:help-screenshots` gate (verify + CI Guards) keeps manifest ↔ PNGs ↔ doc refs in lockstep, rejecting hand-placed (non-regenerable) images and empty alt text (alt is what the Help AI RAG ingest sees). **Capture environment — operator picked "beta + dedicated demo tenant"** over local seeded stack (sparse/fake-looking data, some routes don't render locally) and over existing real tenants (customer PII could ship in published screenshots): tenant `demo` / "Harbor Light Travel" / owner "Dana Whitfield" (fictional), seeded 2026-07-07 on the beta DB (byo_professional tier, active, subscription_status=active), owner auth user `jharvieux+atc-demo@gmail.com` (creds in operator's `.env.local` as `HELP_SHOTS_*`). Session mint uses the GoTrue **admin magic-link API** (generate_link → verify) because email/password logins are disabled on beta — discovered when password grant returned `email_provider_disabled`; enabling the email provider on the prod-serving project was rejected as an unnecessary auth-surface widening. Operator also picked **tooling + all 30 shots** over a 2-doc pilot. Runbook: `docs/runbooks/help-docs.md`. Tooling PR #1685; demo-tenant content seeding, the 30 captures, and PDF/Word-export image handling tracked as follow-up issues (per the deferral rule).

**Rejected**: hand-taken/hand-annotated screenshots (rot on every UI change, non-reproducible style); hosting images off-repo (versioning drift vs docs); post-processing annotations with an image library (new dependency; DOM injection is pixel-deterministic and dependency-free); reusing the `e2e-test` tenant ("E2E Test Tenant" branding would appear in customer docs, and the E2E suite mutates its data).

**Related artifacts**: PR #1685, `docs/runbooks/help-docs.md`, `scripts/help-screenshots/`, `scripts/check-help-screenshots.ts`, D-067 (help docs viewer/export), D-133 (doc-only PR exemption — does NOT apply to screenshot PRs, they carry PNGs + manifest code).

---

## D-323 — 2026-07-08 — 2026-07-01 review backlog swept and merged (#1575–#1621 range): 20 PRs, 3 stale issues closed, 9 new gaps tracked, 3 process fixes shipped

**Decision**: executed `/issue-sweep` against the held 2026-07-01 principal architecture review backlog (D-313/D-315's green light). All 20 approved batches ran to merge — schema constraints (#1575), email idempotency/dedup (#1580–#1582), cron double-send (#1581), Stripe webhook ordering (#1583), auth-transfer defects (#1592), error-sanitization sweep (#1594), escapeHtml/money/HMAC/cache consolidations (#1598/#1606/#1604/#1605), chat perf (#1586), groups hardening (#1600), email-route dedup (#1608), audit-gap triage (#1621), plus D-091 catalog additions (#1612) and two new runbooks (#1603). Real bugs surfaced and fixed mid-merge rather than shipped broken: a Stripe webhook CAS race (missing WHERE-clause guard, #1583), a genuine multi-file merge conflict requiring manual reconciliation twice (#1650's HMAC migration vs #1652's groups-hardening logic; #1643's BoundedTtlCache vs #1651's new snapshot fields — both re-audited Opus-tier after resolution), four independent migration-version collisions from parallel batches picking the same `YYYYMMDD000000` slot, and two separate MEMORY.md D-number collisions from batches computing the same "next" decision number blind to siblings. **Also closed 3 issues merged-but-never-auto-closed** (#1594, #1605 — squash-merge doesn't always trigger GitHub's closing-keyword linkage when the PR touches many files; #1663 — a shared-test-DB migration-ledger hazard that was actually PR #1642's own legitimate pending migration, not an orphan, resolved by merging #1642 rather than running `migration repair`). **Filed 9 follow-up issues** (#1673–#1682) after a dedicated sweep of every merged PR's audit-agent WARNING/NIT comments that had been left untracked at merge time — CLAUDE.md's "every deferral gets an issue" rule was under-applied during the merge cascade itself; the retroactive sweep is the correction, not the intended flow.

**Process fixes shipped from bugs found during this sweep** (each is its own D-entry — cross-referencing, not restating): D-321 (migration-version + MEMORY.md D-number collision prevention, #1660/#1661 → PR #1667), the `post-audit-comment.sh` explicit-PR-number + branch cross-check fix (#1665 → PR #1666 — ambient `gh pr view` resolution let a wrong-worktree cwd post a validly-hashed marker to the WRONG PR, satisfying that PR's gate without ever auditing it), a destructive-git-command prohibition added to both audit agents' Boundaries sections (#1669 → PR #1670 — an audit agent ran `git reset --hard`/`git checkout --` in a shared worktree while "cleaning up" what it misjudged as noise), and merge-train discipline + a `--check` mode on the audit script (#1671 → PR #1672 — investigation found the diff-hash recipe was NOT the source of the repeated re-audit churn during this merge cascade; it already survives no-op `update-branch` merges by design. The actual waste was orchestration-side: update-branching every open PR after every sibling merge instead of once right before that PR's own turn, and dispatching audit agents reflexively instead of checking hash-currency first).

**Rejected**: reworking the audit gate's hash computation (merge-base pinning, tree hashing) — empirically verified the existing recipe already survives no-op merges and correctly invalidates on real content overlap; a recipe change risked weakening real-conflict detection to fix a problem that wasn't the gate's fault. GitHub merge queue adoption — unavailable on a personal-account repo (`mergeQueue: null` via GraphQL), would need an org transfer; deferred, not pursued. Related: D-312/D-313/D-314/D-315 (the sweep skill and backlog-sequencing decisions this session executed against), D-321, #1665, #1669, #1671, `docs/runbooks/pr-workflow.md`, `.claude/commands/issue-sweep.md`.

---

## D-322 — 2026-07-08 — 7 unwired kill-switch/feature-flag env vars resolved: 2 removed, 5 wired (#1668)

**Decision**: resolved issue #1668 (pre-pr-reviewer audit on PR #1644 found 7 env vars in `env.ts` with a Zod schema entry and a boolean-coercion test but zero application-code call sites) per operator's per-flag calls. **Removed** (dead duplicates, no feature behind them): `AI_GLOBAL_KILL_SWITCH` — the real AI kill switch is the DB-backed `ai_kill_switch_state.global_paused` row enforced in `run-supervisor.ts:122-152` and managed via `/api/admin/ai-kill-switch`; the env var duplicated nothing and was never read. `MAINTENANCE_MODE` — no maintenance page exists and nothing checks it; removing rather than building a maintenance-mode feature that wasn't asked for. Both removed from `env.ts`'s Zod schema, their boolean-coercion test cases (`RAG_INGESTION_PAUSED` promoted to replacement false-default canary), `.env.example`, and their `kill-switches-blast-radius.md` entries. **Wired**, each with a request/invocation-time test in both directions (enabled allows, disabled blocks): (1) `OAUTH_GOOGLE_ENABLED`/`OAUTH_MICROSOFT_ENABLED`/`OAUTH_FACEBOOK_ENABLED` — `oauth-initiate/route.ts`'s `ALLOWED_PROVIDERS` was a hardcoded `Set(["google","azure","facebook"])` regardless of the flags; now derived from the three flags at request time via `allowedProviders()`, and the `/signup` page gates each OAuth button's render on the same flag. This also closes Microsoft's dangerous half-wired state noted in the issue — `OAUTH_MICROSOFT_ENABLED`'s boot-time `superRefine` (requiring MS Graph creds) and the login route now consistently agree instead of leaving a live button with no creds under it when the flag was false. Apple intentionally untouched (`OAUTH_APPLE_ENABLED` doesn't gate anything yet — tracked separately in #428, out of scope here). (2) `RAG_INGESTION_PAUSED` — added an early-return skip check (log + `{skipped:true, reason:"rag_ingestion_paused"}`) at the top of `rag-extract-content.ts`, `rag-pii-redact.ts`'s `redactSubmission()`, and `rag-normalize.ts`'s handler — matches the "check first, log, no-op" idiom used elsewhere (e.g. `assertTenantStillPayingById` past-grace skip in the same three files). Documented in the runbook that these are Inngest event-triggered functions, not pollers — a submission skipped while paused does NOT auto-resume when the flag flips back off; something must re-emit the triggering event. (3) `SIGNUP_ENABLED` — `POST /api/auth/signup/complete` returns `403 {error:"signup_disabled"}` before touching the DB when false; `/signup` page renders a "Sign-ups are currently closed" message instead of the OAuth form. (4) `STRIPE_CONNECT_ONBOARDING_ENABLED` — `POST /api/onboarding/connect/link` returns `403 {error:"connect_onboarding_disabled"}` before any Stripe API call when false. **Rejected**: building a maintenance-mode feature to give `MAINTENANCE_MODE` something to gate (not asked for, scope creep on a docs-audit-driven cleanup issue — if maintenance mode is wanted later it should be its own considered feature, not backfilled to justify an existing unused flag). Related: #1668 (closed), #1644, #1603, #428, `docs/runbooks/kill-switches-blast-radius.md`, `docs/runbooks/feature-flags.md`, `docs/runbooks/oauth-providers-setup.md`.

---

## D-321 — 2026-07-07 — Concurrent-agent numbering collisions fixed: real-clock migration versions + MEMORY.md D-number CI guard + hook carve-out (#1660, #1661, PR #1667)

**Decision**: fixed the two numbering-collision classes surfaced by the 2026-07-06 sweep (4 parallel agents deriving the same migration version `20260719000000`; PRs #1652 and #1643 both claiming `D-318`). (1) `scripts/new-migration.sh <app: main|rag> <slug>` replaces hand-picked versions with a real-clock `YYYYMMDDHHMMSS` (second-resolution) version, reserved atomically via `mkdir` in the shared git-common-dir (retry-on-`EEXIST` serializes same-machine concurrent agents) — this is a **convention change** for both apps: main moves off `YYYYMMDD000000`-per-day, and RAG moves off its bare sequential-integer `00NN_slug.sql` scheme entirely (per #1661's finding that RAG's convention has the identical collision structure with *no* date component). (2) `scripts/check-migration-collision.ts` (DB-free, in `pnpm verify` + Guards & Build) catches same-app duplicate versions and branch-vs-`origin/dev` collisions; its baseline file keys on the **exact sorted file list** (`app:version:files`), not just `app:version`, so a third file later landing at an already-baselined version is still a new violation. (3) `scripts/check-migration-ledger.ts`, wired into `deploy.yml`'s `rls-snapshot-diff` job before `supabase db push`, queries the test DB's `supabase_migrations.schema_migrations` ledger directly — the authoritative backstop for a cross-machine race between two branches never diffed against each other. (4) `scripts/check-memory-decision-collision.ts` fails a PR that adds a `## D-NNN` header already claimed on `origin/dev`, plus a `MEMORY.md`/`MEMORY-INDEX.md` header-set consistency check. (5) `.claude/hooks/block-spec-memory-edits.mjs` gained a carve-out allowing an `Edit` that does nothing but consistently renumber one branch-local `D-NNN` token (header + all in-body mentions, verified via exact-substitution equality) to another — but only when that number is absent from `origin/dev`'s `MEMORY.md`; any ambiguity, any edit touching more than a single clean substitution, or an `old_string` that appears verbatim anywhere in `origin/dev`'s file (catches renumbering a token buried in an already-merged entry's *body*, not just its header) falls through to the original block. (6) `.claude/commands/issue-sweep.md` updated so sweep executors never write `MEMORY.md` directly — they return a `memory_entry` JSON field, and the supervisor prepends it serially during per-PR finalization, making the supervisor the sole writer for that flow (the general-case backstop is guard #4). **Found while building this**: a real pre-existing collision, `apps/rag/supabase/migrations/0014_composite_weights.sql` and `0014_platform_settings_sync.sql` both using version `0014` (merged via #105/#108) — filed as **#1664** rather than fixed here (needs live-DB ledger inspection to resolve safely) and baselined so the new gate doesn't fail on it. **Rejected**: collision-proof identifiers (date+PR-number keying, making `D-NNN` a display-only concern) — genuinely collision-proof but disproportionate blast radius (`[[D-NNN]]` cross-refs throughout `MEMORY.md`, the index rebuild snippet, the hook, `/memory-entry`, operator habit) for a problem the layered fix already closes; supervisor-side D-number range pre-reservation as the *general* fix — rejected because it only covers sweep-shaped concurrency and couples numbering to one skill file (adopted only as the sweep-specific single-writer piece, item 6 above). Related: #1660, #1661, #1664 (new), PR #1667, `docs/runbooks/migrations.md` (new rule 0), `.claude/commands/memory-entry.md`. **Note**: this very entry collided with #1643's D-320 during merge (ironic, given the subject) — renumbered to D-321 as part of PR #1667's own conflict resolution, which is exactly the manual process this decision's tooling now automates going forward.

---

## D-320 — 2026-07-07 — Bounded TTL-cache utility added: lib/cache/bounded-ttl-cache.ts (1000 entries, 5min default TTL)

**Decision**: Issue #1605 (2026-07-01 architecture review, "5 unbounded hand-rolled TTL caches") is closed via a new shared utility `lib/cache/bounded-ttl-cache.ts`: `class BoundedTtlCache<K,V>` with LRU eviction (max 1000 entries configurable), per-entry TTL (default 5min, overridable per-call), and `.get()/.set()/.delete()/.clear()` methods. All 5 hand-rolled caches migrated (resolve-tenant 4 maps, vendor-health/registry, abuse/snapshot, personas 2 caches, rag/entity-extraction). Existing TTLs preserved (60s/30s/1h per site); only the entry-count bound is new. **Why**: Fluid Compute instances are long-lived; unbounded Maps on instance-local middleware path are a slow-leak vector. A centralized, tested utility is the durable fix (vs. repeating per-file). The 1000-entry bound is a threshold assumption; it's untested against real cardinality for user-auth-keyed caches, so future scaling-under-load should re-check this and potentially override `maxEntries` per site. **Rejected**: (a) using the `lru-cache` npm package (introduces a runtime dependency, which requires operator approval per CLAUDE.md; the shared utility avoids that). (b) Migrating vendor-health/registry.ts's separate `memRegistry` Map (durable state, not a TTL cache, bounded by fixed vendor enum — out of scope). (c) Migrating pricing-table.ts and stripe/price-ids.ts (both still bare TTL caches; left for future scope). Related: PR #1643, test file `apps/main/test/unit/cache/bounded-ttl-cache.test.ts` (17 tests covering LRU eviction, TTL expiry, miss vs. cached-null distinction).

---

## D-319 — 2026-07-07 — Money-formatter consolidation redone clean (#1638 → #1657); fromCents brand-type kept narrow

**Decision**: PR #1638 (batch 8f, money-formatter consolidation onto `lib/money.ts`) was cancelled after audit found its single commit only added `formatCents()` + one dead import — none of the claimed 12 call-site migrations were actually in the diff, and it had swept in unrelated doc changes (CLAUDE.md anti-patterns 21–26, `d091-reviewer.md`, `anti-patterns.md`). Rather than patch the stale branch, cut a fresh `feature/money-consolidation-1638` off `dev` and redid the consolidation for real: local `dollars()`/`formatPrice()`/`formatMoneyCents()`/inline `cents/100` patterns removed from 14 files, all routed through the new canonical `formatCents(cents, currency?)`. Two bugs the audit had also flagged were fixed in the redo: (1) admin/pricing's `<input type="number">` fields were still wired to the new `formatCents()` (a currency string like `"$12.34"`), which blanks number inputs — switched those specific call sites to `fromCents()` (plain decimal string) instead; (2) dead imports cleaned up. Went through **three rounds** of the d091-reviewer/pre-pr-reviewer audit loop before merge: round 1 found `fromCents` widened from `Cents | bigint` to also accept plain `number` — a brand-type erosion, since the whole point of the `Cents` brand is that only validated values reach money-display code — plus missing test coverage for the new `formatCents` export and an under-audited 13th formatter (`formatDollars` in admin/resources, which has genuinely different display rules — sub-cent `<$0.01` + magnitude-based precision — and was kept separate, just documented inline). Round 2's fix (casting `cents as any as Cents` at call sites to avoid re-widening the shared function) was itself flagged as worse than the original widening — an `any`-intermediary silences type-checking for the whole expression. Round 3 fixed it properly: `BigInt(Math.round(cents)) as Cents`, a real runtime conversion, no `any`. Also surfaced a pre-existing bug (not introduced by this PR, inherited from all the original formatters): `formatCents` always divides by 100 regardless of currency, so zero-decimal currencies (JPY, KRW) render 100x too small — filed as **#1658** rather than fixed inline, since a real fix needs a product decision on how non-2-decimal-currency cents are stored; a regression test documents the current (buggy) behavior so it can't silently change. **Rejected**: fixing the JPY bug in-PR (scope creep on an unrelated pre-existing issue with no clear correct behavior yet); keeping the `fromCents(number)` widening (brand-type safety > convenience — this is exactly the kind of guard rail D-091 "no stub-shaped code" catalog exists to protect); carrying over #1638's stranded doc changes (unrelated to the issue, left on `feature/sweep-money-1606` for the operator to decide separately — that branch was NOT deleted, only the corrective PR's branch was). Related: #1606, #1638 (closed, superseded), #1657 (merged), #1658 (new, JPY formatCents bug), `apps/main/src/lib/money.ts`, `apps/main/test/unit/money.test.ts`.

---

## D-318 — 2026-07-06 — Groups hardening (#1600): compensating delete over RPC/transaction; partial unique index for invite dedup

**Decision**: fixed two defects on `feature/sweep-groups-1600` (batch 11a). (1) `POST /api/groups`: an `invitations` insert failure after the `groups` row is already `status:'active'` previously 500'd and left an orphaned, empty, un-retryable-without-duplicating group. Fixed with a **best-effort compensating delete** (`groups.delete().eq(id).eq(tenant_id)` on insert failure, logged if it itself fails) rather than wrapping the whole create in a SECURITY DEFINER RPC/transaction — the single INSERT for `invitations` is already atomic (all-or-nothing per statement, so no partial rows exist on failure), and `forums.group_id` is `ON DELETE CASCADE`, so deleting the group also cleans up the auto-created forum row. An RPC would add a migration + function-grant surface for a failure path that's rare and fully undoable via one delete. (2) `POST /api/groups/[id]/invitations` (single-invite) bypassed the create path's 50-cap, the §27.4 `incrementGroupInvitees` counter, and had no dedup — same address invitable repeatedly. Added a pre-insert `COUNT` cap check (`>= 50` → 400), the same counter-increment call the create path makes (non-fatal on failure, matching precedent), and a **partial** unique index `invitations_group_active_email_uniq_idx ON invitations(group_id, lower(invitee_email)) WHERE token_revoked_at IS NULL` (migration `20260719000000_invitations_active_email_dedup.sql`, with a dedup `DELETE` of existing active duplicates first) — partial so historical revoked rows and `reissue_all`'s revoke-then-reinsert flow never collide with it; a live 23505 on insert now returns a friendly `409 invitee_already_invited` instead of a raw 500. **Rejected**: (a) a non-partial unique index — would break `reissue_all` the first time a group has ever had a duplicate-email invite lifecycle; (b) adding new email-send rate-limiting to the single-invite path — the batch instructions mentioned "respect rate-limit," but `group_invitation`-category sends already skip `checkRateLimit`'s per-category throttle by design (the actual 3/24h cap lives in `group-reminder-cadence.ts`'s cron, a separate concern) — no new gate added since the issue's acceptance criteria only named cap/counter/dedup. Three existing test files needed mock updates because the insert path moved off `safeAwait` (needed direct access to the Postgres error code for the 23505→409 branch) — `group-invitation-send.test.ts`'s mock had a "safeAwait intercepts this" broken-thenable trick that started hanging once the real code stopped routing through the mocked `safeAwait`. Related: #1600, `apps/main/src/app/api/groups/route.ts`, `apps/main/src/app/api/groups/[id]/invitations/route.ts`, `db/rls-exceptions.txt` (unaffected — no policy/grant change, so no snapshot regen needed).

---

## D-317 — 2026-07-06 — Audit-model criteria rebased for Sonnet 5: Opus split by trigger type, not by agent

**Decision** (operator ruling, after pushback on a weaker proposal): Opus overrides for the FIRST audit run are now split by **trigger type** — risk triggers (SQL migration, net-new route/Inngest/cron, webhook signatures/idempotency/state-machine, new service-role path) get Opus for **BOTH** agents; the size-only trigger gets Opus for **d091-reviewer only** (pre-pr-reviewer stays Sonnet), with the size bar raised from D-147's ≥10 files/≥500 net lines to **≥20 files/≥1000 net lines** for Sonnet 5. Re-runs stay Sonnet for both. **Why**: pre-pr's tests-for-intent and honesty-about-uncertainty checks are genuinely judgment-heavy and matter most on exactly the risky diffs — the agents are the operator's only review bench, so don't thin defense-in-depth where the risk lives; but pre-pr's checks don't scale with raw diff size the way d091's blast-radius tracking does, so "big but boring" diffs upgrade only d091. **Rejected**: (a) always-Sonnet for pre-pr on all triggers (my initial proposal — operator correctly challenged it; tests-for-intent on a payment-path diff is not shallow work); (b) dropping the size trigger entirely (audits are the sole reviewer — keep a backstop); (c) leaving D-147 unchanged. Supersedes the audit-model half of D-147 and the "PR-audit selection intentionally unchanged" note in D-314. Related: pr-workflow.md "Model selection", PR #1628, [[D-316]].

---

## D-316 — 2026-07-06 — Audit-agent flow simplified: parallel agents concurrent with CI, summary comments only, shared posting script, expanded d091 coverage

**Decision** (operator-approved process review): (1) `d091-reviewer` and `pre-pr-reviewer` are now fully independent — launch both **in parallel** (single message) **immediately after `gh pr create`, concurrent with CI** (supersedes D-270's run-agents-after-CI ordering; rationale: `pnpm verify` is mandatory pre-push and is a superset of most required CI jobs, so post-push CI failures are rare — overlapping agent time with CI time wins the expected case, and the worst case (fix-commit → re-run both) matches the old flow). (2) The PR-body `## Audit` block is **retired** — nothing writes it; the hash-bound marker comments remain the only gate (`pr-audit-section-check` unchanged). Agents post **summary** comments (scope + finding one-liners + standalone Status) and return full findings to the invoking session. (3) Posting mechanics (PR resolution, diff-hash, comment) extracted to `scripts/post-audit-comment.sh` — hash recipe now lives in exactly two places (script + workflow) that must stay byte-identical, down from three. (4) Check ownership de-duplicated: slop + orphan-TODO belong to pre-pr-reviewer/lint only (dropped from d091); stub-shaped code belongs to d091 only (dropped from pre-pr); both agents skip gate-owned greppable forms and instead audit indirection + new escape-hatch/baseline entries. (5) d091-reviewer gained six patterns: Inngest side-effects-outside-step.run (BLOCKER), module-level mutable state in serverless paths (beyond G4's limiter-name heuristic), date-only-vs-timestamp handling, PII in server logs, index coverage for new query shapes on user-growing tables, and grant-widening delta reporting on `permission-grants.ts` changes. (6) Both agents support **local mode** (pre-PR report-only, posts nothing, never satisfies the gate) for shift-left on high-risk diffs. Also: removed the third-party vitals plugin's prompt-type Stop hook from its cached hooks.json (fired uselessly on every no-change turn; unsupported to disable per-hook — a plugin update restores it, re-delete then). **Rejected**: merging the two agents into one run (loses independent coverage); a deterministic body-block upsert script (operator doesn't need the body block); implementing #1612's six catalog patterns here (D-315 routes that backlog through /issue-sweep — commented on #1612 instead). Related: PR for this change, D-270, D-147, #1612.
...
**The three guards.** (G3) `counter-rmw` — a NEW detector in `check:d091` (not a standalone script — the epic said "codify as a D-091 pattern"); flags `.update({ <counter/balance/quota>: <var> ± … })` non-atomic read-modify-write (F-pay-01/F-sm-01/02); 7 pre-existing sites baselined. (G4) `scripts/check-inmemory-rate-limit.ts` (`check:rate-limit-store`) — flags a module-level `new Map()/Set()` whose **variable name** reads like a limiter (`rate|limit|throttle|attempt|bucket|quota|hits`), separating limiters from the many in-process caches; 2 baselined (public-chat rateMap = F-tok-01 #1386; MS-OTP ipAttempts = accepted-risk #735). (G6) `scripts/check-webhook-replay.ts` (`check:webhook-replay`) — flags a handler that READS an inbound signature (`headers.get("…signature")`/`constructEvent`/`verify*Signature(` — NOT bare `timingSafeEqual`, which also matches admin-key/cookie compares) but has no replay signal; scans lib too so the Stripe delegation route passes via its imported handler; `*-signature.ts` verifier helpers exempt; only `feedback/route.ts` baselined (#1385).

**Why / lessons.** (a) All three are heuristic text/name matchers with baseline + inline escape hatch (`d091-allow:counter-rmw`, `inmem-ratelimit-allow:`, `webhook-replay-allow:`) — same conservative shape as the existing service-role-tenant detector; honest about being presence-checks, not correctness proofs (the d091-reviewer + fixture tests cover correctness). (b) Audit-agent value proven on G6: d091-reviewer caught that bare `\btimestamp\b` in REPLAY_RE was too broad (any `created_timestamp` column silently exempts) AND that the `source_revision >= incoming` guard in tenant-events/platform-settings-events genuinely defeats replay (the `>=` no-ops even the latest revision) — so I'd wrongly filed #1410; closed it not-a-bug, added `source_revision` as a recognized signal, baseline shrank 3→1. (c) Two pre-existing bugs surfaced + ticketed, not fixed inline: #1408 (detectServiceRoleTenant false-negative — a `.select(...tenant_id...)` column list masks a missing `.eq("tenant_id")` filter). Each guard PR ran the full agent cycle (CI green → d091 → pre-pr → fix nits → re-run).

**Rejected.** G3 as a standalone script (the d091 check is the idiomatic home for a D-091 pattern detector, auto-rides its CI step). G4/G6 as d091 detectors (they're distinct concerns with their own baselines — standalone scripts match the G1/G2/G5 precedent). For G6's two revision-guarded routes: inline `webhook-replay-allow:` + remove from baseline (rejected in favor of teaching the guard `source_revision` — no app-code edit, generalizes).
