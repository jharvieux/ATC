# Build Prompt — Populate Help Docs into RAG (v1)
...
2. An ingestion script (`scripts/ingest-help-docs.ts`) that processes the Markdown files into chunks.
3. Integration with the existing ingestion pipeline (v6 §22.4) for PII redaction (defensive — the docs shouldn't contain PII but the pipeline runs anyway).
4. A GitHub Actions workflow that re-ingests on every release that touches `apps/main/content/help/**`.
5. Tests that confirm retrieval works against the new scope.
...
2. Read v6 §22 in full, especially §22.4 (normalization pipeline) and §22.6 (revised global review).
3. Read addendum §3.4 (platform-docs scope authorization).
4. Inspect the existing RAG service code at `apps/rag/` (per CI/CD doc) — specifically:
...
5. Produce a written plan in chat covering:
...
2. Parse frontmatter.
3. Chunk per the strategy above.
4. Run each chunk through PII redaction (fail loud on detection).
5. Generate embeddings (use the same embedding model v6 §6 specifies).
6. Upsert chunks into `knowledge_chunks` keyed by `(scope, source_url, chunk_index)`. Upsert, not insert, so re-running the script on updated docs replaces existing chunks rather than duplicating.
7. After upsert, delete any platform-docs chunks whose `source_url` no longer corresponds to a file in the docs directory (handles deleted docs).
8. Log a summary: files processed, chunks created, chunks updated, chunks deleted.
...
3. Run the ingest script against the appropriate environment's RAG service.
4. Report summary as workflow output.
5. On failure: surface the error in the Action summary AND open a GitHub issue tagged `help-docs-ingestion-failed` so it doesn't get silently missed.
